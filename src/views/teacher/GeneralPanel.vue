<template>
  <div style="padding-left: 1rem; padding-right: 1rem;">
    ...
    <n-space vertical size="large">
        ...
        <n-layout>
            <n-infinite-scroll style="height: 350px;border-right: 1px solid black; border-top: 15px solid #0D5A79; border-radius: 10px 10px 0px 0px;">
                <n-grid :cols="3" :x-gap="16" :y-gap="16" style="margin-top:1rem;">
                    <n-grid-item v-for="curso in data" :key="curso.id">
                        <n-card hoverable style="height: 100px; border-left: 4px solid blue;"><p class="multi-ellipsis">{{ curso.name }}</p></n-card>
                    </n-grid-item>
                </n-grid>
            </n-infinite-scroll>
            <n-pagination :page-slot="8" v-model:page="page" :page-count="100" style="background-color: #0D5A79; padding: 8px; margin-top: 10px; justify-content: center;"/>
        </n-layout>
    </n-space>
  </div>
</template>
...
<script scoped>
import { NSpace,NBreadcrumb,NBreadcrumbItem,NIcon, NLayout, NInput, NFormItem, NCard, NGrid, NGridItem, NInfiniteScroll, NPagination } from 'naive-ui';
